...
const CreateNewForm = ({data}) =>{
    ...
    for (var dataset of data.datasets){
      datasets.push({id:JSON.parse(dataset).id,
                      name: JSON.parse(dataset).name})
    }
    ...
    const handleSubmit = (event) =>{
        ...
        PostData('http://localhost:5000/survey', JSON.stringify(allValues)).then(
          (response) => {
    ...
    console.log(JSON.parse(data.datasets[0]))
    return (
